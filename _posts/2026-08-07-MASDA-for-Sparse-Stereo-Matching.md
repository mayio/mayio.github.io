...
this post applies MASDA there — to the *dense* problem, a disparity for every
pixel — and measures what it gains.

The formulation runs on **sparse matrices**. Each pixel offers only its two best
disparity candidates out of the aggregated cost volume, so the association graph
carries two edges per pixel instead of a $$W \times W$$ matrix per row, and every
...
*Revision note, 2026-08-09: this article originally applied MASDA to sparse
**keypoint** matching. That application measured itself out of a job — its own
analysis showed detector repeatability bounds recall below 51% before matching
starts, a ceiling the dense formulation simply deletes — so the page now presents
what replaced it: dense MASDA on sparse matrices, re-measured. The keypoint study
remains in the site's git history.*
...
For stereo, substitute the nouns — per rectified image row. Pixels in the left
...
from the left camera is a misdetection ($$\gamma$$). Occlusion is not a small
...
`uint64`, so the distance is a single `popcount`, and Census is invariant to
...
the Hamming distance $$h$$ around that point gives a score with a usable zero:
...
signal per pixel. The score MASDA actually consumes is *aggregated* over an
edge-aware support region — an O(N) recursive filter that stops at intensity
...
The matcher is measured on **Teddy** and **Cones** from the Middlebury 2003
stereo set, and on the six Middlebury 2005 scenes: Art, Books, Dolls, Laundry,
Moebius and Reindeer, at third size. They are rectified, and they ship
structured-light ground truth at quarter-pixel resolution with a few percent of
...
most to contribute. This is the dense-problem version of the original keypoint
study's central finding — the constraint pays in proportion to the ambiguity —
now measured on three orders of magnitude more answers.

For the full engineered pipeline (the C++ implementation with its margin gate,
which trades a little coverage for precision), the comparison against OpenCV's
SGM lands at **9.7% bad-1.0 against SGM's 10.9%** at 76% versus 78% coverage —
...
The complexity argument is $$O(T \cdot E)$$ against Jonker-Volgenant's
...
is anytime, incremental, and accepts factors that destroy the assignment
structure, where a LAP solver cannot follow.
...
**Jonker-Volgenant / Hungarian.** Exact, $$O(N^3)$$, and here exactly as good as
...
**Semi-global matching.** The standard fast dense method, and the direct
competitor now that this formulation is dense too. SGM aggregates smoothness
along scanline paths and has **no uniqueness constraint at all** — a left-right
consistency check is bolted on afterwards, which costs a second matcher run.
MASDA gets mutual exclusivity inside the inference, in one run, plus a per-pixel
confidence (the margin) as a by-product. Measured head-to-head in Part 2:
...
**ELAS** narrows a dense search around triangulated support points — the
avoid-the-sweep family. Part 2 measured this project's version of that idea (a
coarse-to-fine mask) at accuracy parity and recorded exactly where its speedup
goes to die on embedded hardware.
...
**Sub-pixel disparity.** The candidates are integer; the fit to the correlation
